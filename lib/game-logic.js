...
//
// Two cooperative modes, both built on the same consensus mechanic (players
// type freely, no locking in, a round clears once enough distinct players
// have independently typed the same word):
//   "sync"   — one emoji falls, target keywords are its own list.
//   "double" — two emojis fall together, target keywords are only the ones
//              they share, so the room has to find a word that fits both.
...
function createRoomState(code, mode, consensusLevel) {
  return {
    code,
    players: {},
...
async function spawnEmoji(room) {
  ...
  const { EMOJI_DB, EMOJI_LIST, SHARED_PAIRS } = await getEmojiSource();

  let emojiForRound;
  if (room.mode === "double") {
    const pair = randChoice(SHARED_PAIRS);
    emojiForRound = [pair.a, pair.b];
    room.targetKeywords = pair.shared;
  } else {
